...
def check_user_balance(conn, user_id, required_amount):
...
def create_order(conn, user_id, product_id, total):
    order_id = save_order_db(conn, user_id, product_id, total)
    return order_id
...
def notify_user(conn, user_id, order_id, total):
...
def process_order(conn, order_data):
    ...
    check_user_balance(conn, order_data["user_id"], final_total)

    order_id = create_order(conn, order_data["user_id"], order_data["items"], final_total)

    notify_user(conn, order_data["user_id"], order_id, final_total)
